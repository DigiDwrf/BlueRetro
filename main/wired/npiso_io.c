...
static unsigned npiso_sfc_snes_5p_isr(unsigned cause) {
    ...
    /* Set first bit */
    if (high_io & NPISO_LATCH_MASK) {
        set_data(0, 0, (wired_adapter.data[0].output[0] | wired_adapter.data[0].output_mask[0]) & 0x80); /* 1p */
        idx[0] = 0;
        mask[0] = 0x40;
        set_data(1, 1, 0); /* Multitap presence */
        switch (GPIO.in & P2_SEL_MASK) {
            default:
                set_data(1, 0, (wired_adapter.data[1].output[0] | wired_adapter.data[1].output_mask[0]) & 0x80); /* 2p */
                idx[1] = 0;
                mask[1] = 0x40;
                while (GPIO.in1.val & NPISO_LATCH_MASK); /* Wait falling latch edge */
                set_data(1, 1, (wired_adapter.data[2].output[0] | wired_adapter.data[2].output_mask[0]) & 0x80); /* 3p */
                break;
            case 0:
                set_data(1, 0, (wired_adapter.data[3].output[0] | wired_adapter.data[3].output_mask[0]) & 0x80); /* 4p */
                idx[1] = 0;
                mask[1] = 0x40;
                while (GPIO.in1.val & NPISO_LATCH_MASK); /* Wait falling latch edge */
                set_data(1, 1, (wired_adapter.data[4].output[0] | wired_adapter.data[4].output_mask[0]) & 0x80); /* 5p */
                break;
        }   
    }

    while (!(GPIO.in1.val & NPISO_LATCH_MASK)) {
        /* Update port 0 data lines on rising clock edge */
        if (low_io & P1_CLK_MASK) {
            ...
            switch (idx[0]) {
                case 0:
                    ...
                    set_data(0, 0, (wired_adapter.data[0].output[0] | wired_adapter.data[0].output_mask[0]) & mask[0]);
                    mask[0] >>= 1;
                    break;
                case 1:
                    while (!(GPIO.in & P1_CLK_MASK)); /* Wait rising edge */
                    set_data(0, 0, (wired_adapter.data[0].output[1] | wired_adapter.data[0].output_mask[1]) & mask[0]);
                    mask[0] >>= 1;
                    break;
                default:
                    while (!(GPIO.in & P1_CLK_MASK)); /* Wait rising edge */
                    set_data(0, 0, 0);
                    break;
                /* Controller detected on 17th cycle */
            }
        }

        /* Update port 1 data lines on rising clock edge */
            ...
            switch (idx[1]) {
                case 0:
                    switch (GPIO.in & P2_SEL_MASK) {
                        default:
                            ...
                            set_data(1, 0, (wired_adapter.data[1].output[0] | wired_adapter.data[1].output_mask[0]) & mask[1]);
                            set_data(1, 1, (wired_adapter.data[2].output[0] | wired_adapter.data[2].output_mask[0]) & mask[1]);
                            break;
                        case 0:
                            while (!(GPIO.in & P2_CLK_MASK)); /* Wait rising edge */
                            set_data(1, 0, (wired_adapter.data[3].output[0] | wired_adapter.data[3].output_mask[0]) & mask[1]);
                            set_data(1, 1, (wired_adapter.data[4].output[0] | wired_adapter.data[4].output_mask[0]) & mask[1]);
                            break;
                    }
                    mask[1] >>= 1;
                    break;
                case 1:
                    switch (GPIO.in & P2_SEL_MASK) {
                        default:
                            while (!(GPIO.in & P2_CLK_MASK)); /* Wait rising edge */
                            set_data(1, 0, (wired_adapter.data[1].output[1] | wired_adapter.data[1].output_mask[1]) & mask[1]);
                            set_data(1, 1, (wired_adapter.data[2].output[1] | wired_adapter.data[2].output_mask[1]) & mask[1]);
                            break;
                        case 0:
                            while (!(GPIO.in & P2_CLK_MASK)); /* Wait rising edge */
                            set_data(1, 0, (wired_adapter.data[3].output[1] | wired_adapter.data[3].output_mask[1]) & mask[1]);
                            set_data(1, 1, (wired_adapter.data[4].output[1] | wired_adapter.data[4].output_mask[1]) & mask[1]);
                            break;
                    }
                    mask[1] >>= 1;
                    break;
...
void npiso_init(void)
{
    ...
    if (dev_type[1] == DEV_SFC_SNES_MULTITAP) intexc_alloc_iram(ETS_GPIO_INTR_SOURCE, 19, npiso_sfc_snes_5p_isr);
    else intexc_alloc_iram(ETS_GPIO_INTR_SOURCE, 19, npiso_isr);
}